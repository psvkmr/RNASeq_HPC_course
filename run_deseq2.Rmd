...
# set your own path to the folder containing the gene counts matrix
...
Four libraries are required for following this differential gene expression analysis guide:
...
Run the above chunk and run *sessionInfo* in the console to make sure the libraries are now loaded

If there is an error loading the package, make sure the package is listed when you run *installed.packages()* \
If not present, install the packages from the console with the examples above.

## Input data

Two input data files are required for this guide, the RNA-Seq reads counts matrix and the samples metadata file
...
The names are the entire file paths from Rosalind, we will set them to the sample names as we load the metadata

```{r cm}
counts.dataframe <- read.table('gene_counts.tab', header = T, row.names = 1)
...
The accompanying metadata for the pasilla dataset, filtered to only include the variables of interest
...
Example experiment designs below. design.formula shows the design formula modelling by differences in the condition variable only, whereas alt.design.formula models for both type and condition variables, with the results of analysis being based on differences attributed specifically to the condition variable.
...
A model matrix can be used to show how the provided design formula will be used to control the use of each variable in the metadata in the modelling process.

```{r model_matrix}
modelled.matrix <- model.matrix(design.formula, coldata)
modelled.matrix
```
...
pca.plot <- plotPCA(vst.matrix, intgroup = c('isolate'))
pca.plot
```
...
ggplot(as.data.frame(deseq.results), aes(log2FoldChange, -log10(pvalue))) +
  geom_point(aes(colour = ifelse(padj < 0.05 & abs(log2FoldChange) > 1.5,
                                 ...
                                 'None')), 
             alpha = 0.2) +
  scale_colour_manual(name = 'Significant differential expression',
